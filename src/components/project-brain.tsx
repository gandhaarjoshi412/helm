...
export function ProjectBrain() {
  ...
  return (
    <section id="brain" className="relative py-24 bg-[#08090c] border-t border-white/[0.06] overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl space-y-4 mb-16">
          ...
          <p className="text-base sm:text-lg text-zinc-400 leading-relaxed">
            HELM constructs and updates a continuous knowledge graph of your entire repository. When you or an agent asks a question, HELM navigates the true architecture — tracing imports, call-stacks, environment configs, and documentation in milliseconds.
          </p>
        </div>
            ...
            {queries.map((q, idx) => (
              <button
                ...
                className={cn(
                  ...
                  selectedQuery === idx
                    ? "bg-sky-500/15 text-sky-200 border-sky-500/40 shadow-[0_0_20px_rgba(56,189,248,0.1)] font-semibold"
                    : "bg-zinc-900/80 text-zinc-400 border-white/[0.06] hover:border-white/20 hover:text-zinc-200"
                )}
              >
                <FileSearch className="w-3.5 h-3.5 text-sky-400 shrink-0" />
                <span>“{q.prompt}”</span>
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          ...
          <div className="lg:col-span-7 rounded-2xl bg-[#0c0e14] border border-white/[0.08] p-6 flex flex-col justify-between shadow-2xl relative overflow-hidden">
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          active
                            ? "bg-sky-500/20 border-sky-400/50 text-white shadow-[0_0_12px_rgba(56,189,248,0.2)] font-semibold"
                            : "bg-zinc-900/40 border-white/[0.04] text-zinc-400"
                        )}
                      >
                        <div className="flex items-center justify-between">
                          <span>{node.name}</span>
                          {active && <span className="w-1.5 h-1.5 rounded-full bg-sky-400 animate-ping" />}
                        </div>
            ...
            <div className="pt-3 border-t border-white/[0.06] flex items-center justify-between text-xs font-mono text-zinc-400">
              <span>Model: <strong className="text-zinc-200">HELM AST Graph Engine</strong></span>
              <span className="text-sky-300">Continuous 0ms incremental re-index</span>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-3 font-mono text-xs">
                {current.trace.map((step, idx) => (
                  <div
                    key={idx}
                    className="p-3 rounded-xl bg-zinc-950/90 border border-white/[0.06] relative group hover:border-sky-500/30 transition-colors"
                  >
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        <span className="w-5 h-5 rounded-full bg-sky-500/10 border border-sky-500/30 text-sky-300 text-[10px] flex items-center justify-center font-bold">
                          {idx + 1}
                        </span>
                    ...
                    <div className="text-[11px] text-sky-300 pl-7 font-mono">
                      {step.file}
                    </div>
              ...
              <div className="mt-4 p-3.5 rounded-xl bg-sky-950/20 border border-sky-500/20 text-xs text-zinc-300 leading-relaxed font-sans">
                <strong className="text-sky-300 font-mono text-xs block mb-1">HELM Summary:</strong>
                {current.explanation}
              </div>
            </div>
