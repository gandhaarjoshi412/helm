...
export function CommandPalette({ isOpen, onClose, onOpenAccessModal }: CommandPaletteProps) {
  ...
  const actions = [
    ...
    {
      id: "access",
      title: "Request HELM Command Access",
      action: () => {
        onClose();
        onOpenAccessModal();
      },
    ...
    >
      ...
      >
            ...
            filtered.map((item) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <ArrowRight className="w-3.5 h-3.5 text-zinc-600 group-hover:text-sky-400 group-hover:translate-x-0.5 transition-transform" />
                </div>
              </button>
            ))
          )}
        ...
        <div className="px-4 py-2 border-t border-white/[0.06] bg-zinc-950/80 flex items-center justify-between text-[11px] font-mono text-zinc-500">
          <span>HELM Command Dispatch</span>
          <div className="flex items-center gap-3">
            <span>↑↓ Navigate</span>
            <span>↵ Select</span>
          </div>
